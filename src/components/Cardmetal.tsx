...
const Cardmetal = () => {
  ...
  const handleWhatsApp = (productName: string): void => {
    ...
    const brandName = "Atap Prima Roof";
  ...
  return (
    <section className="font1 text-black p-4 relative mt-20">
      <header className="text-center md:text-left">
        <h2 className="text-xl font-bold">Produk Rangka Baja Ringan dan Atap Metal</h2>
      </header>
      ...
      <article className="border border-secondary rounded-lg p-4 mt-2 pb-20 relative">
        <header className="absolute left-4 top-4 flex items-start space-x-4">
          <img src="/images/Primasupreme/logo.png" alt="Logo Prima Supreme" className="w-20 md:w-28 h-auto" />
          <div>
            <h3 className="text-md font-semibold">Atap Prima Roof</h3>
            <p className="text-xs">Atap Prima Roof berkualitas tinggi dengan daya tahan terhadap cuaca ekstrem.</p>
          </div>
        </header>
          ...
          {products.slice(startIndex, startIndex + itemsPerPage).map((product) => (
            <article key={product.id} className="w-full md:w-1/3">
              <div className="group border border-secondary rounded-lg overflow-hidden transition duration-300 ease-in-out hover:bg-secondary">
                <img src={product.image} alt={product.name} className="w-full h-48 md:h-64 object-cover object-center" />
                <div className="p-4 flex flex-col justify-between h-full group-hover:text-white">
                  <h3 className="text-lg md:text-xl font-bold text-gray-800 mb-2 group-hover:text-white">{product.name}</h3>
                  <p className="text-gray-600 mb-4 group-hover:text-white">Atap berkualitas tinggi untuk rumah dan bangunan modern.</p>
                  <button
        ...
        <nav className="absolute bottom-4 right-4 flex gap-2 items-center">
          ...
          <button onClick={prevSlide} className="p-3 text-white bg-secondary/75 rounded-full hover:bg-secondary/95 shadow-md">
            <IconChevronLeft size={30} />
          </button>
          <button onClick={nextSlide} className="p-3 text-white bg-secondary/75 rounded-full hover:bg-secondary/95 shadow-md">
